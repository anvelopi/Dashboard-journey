...
<div class="tab-pnl" id="tab-clusters"><div class="dashboard">
<div class="card full"><div class="card-title"><span class="dot"></span>Clusters sem\u00e1nticos \u00b7 <span id="clusterCount" style="color:var(--accent)"></span></div><div id="clusterContent"></div></div>
</div></div>
...
function fil(){return KW.map(function(k){k.score=kwScore(k);return k}).filter(function(k){if(k.archs&&!k.archs.some(function(a){return aA.has(a)}))return false;if(aP!=="all"&&k.phase!==aP)return false;if(sT&&k.kw.toLowerCase().indexOf(sT.toLowerCase())===-1)return false;return true})}
...
function updTbl(d){var hdr=document.querySelector('#kwTable thead tr');var cols=[{k:'kw',l:'Keyword'},{k:'phase',l:'Fase'},{k:'intent',l:'Intent'},{k:'clicks',l:'Clicks'},{k:'imp',l:'Imp'},{k:'kd',l:'KD'},{k:'pos',l:'Pos'},{k:'score',l:'Score'}];hdr.innerHTML=cols.map(function(c){return '<th data-col="'+c.k+'">'+c.l+(sC===c.k?(sD>0?' \u25b2':' \u25bc'):'')+'<\/th>'}).join('');var sorted=d.slice().sort(function(a,b){var va=a[sC],vb=b[sC];if(typeof va==='string')return(va||'').localeCompare(vb||'')*sD;return((va||0)-(vb||0))*sD});document.getElementById('tableCount').textContent=sorted.length+' keywords';document.querySelector('#kwTable tbody').innerHTML=sorted.map(function(k){var sc=kwScore(k);var pc=k.pos<=5?'color:var(--success)':k.pos<=10?'color:var(--warn)':'color:#dc2626';return '<tr><td>'+k.kw+'<\/td><td><span class="pill" style="background:rgba(9,9,11,.06)">'+(k.phase||'')+'<\/span><\/td><td><span class="pill" style="background:var(--surface2)">'+(k.intent||'')+'<\/span><\/td><td style="text-align:right;font-family:var(--mono);font-weight:600">'+k.clicks+'<\/td><td style="text-align:right;font-family:var(--mono)">'+k.imp.toLocaleString()+'<\/td><td style="text-align:right;font-family:var(--mono)">'+(k.kd||'\u2014')+'<\/td><td style="text-align:right;font-family:var(--mono);font-weight:600;'+pc+'">'+(k.pos||0)+'<\/td><td style="text-align:right;font-family:var(--mono);font-weight:600;color:'+(sc>=70?'var(--success)':sc>=40?'var(--warn)':'var(--text3)')+'">'+sc+'<\/td><\/tr>'}).join('')}
...
function updRev(d){var el=document.getElementById('revenueContent');if(!el)return;var cr=0.0185,tk=224.85;var ctrByPos=function(p){if(p<=1)return 0.396;if(p<=2)return 0.187;if(p<=3)return 0.112;if(p<=5)return 0.068;if(p<=10)return 0.032;if(p<=20)return 0.015;return 0.005};var rows=d.filter(function(k){return(k.vol||k.imp)>50}).sort(function(a,b){return(b.vol||b.imp)-(a.vol||a.imp)}).slice(0,30).map(function(k){var vol=k.vol||k.imp||0;var curCtr=k.pos>0?ctrByPos(k.pos):0;var topCtr=ctrByPos(2);var curRev=Math.round(vol*curCtr*cr*tk);var potRev=Math.round(vol*topCtr*cr*tk);var delta=potRev-curRev;return '<tr><td>'+k.kw+'<\/td><td style="text-align:right;font-family:var(--mono)">'+vol.toLocaleString()+'<\/td><td style="text-align:right;font-family:var(--mono)">'+(k.pos>0?k.pos.toFixed(1):'\u2014')+'<\/td><td style="text-align:right;font-family:var(--mono)">\u20ac'+curRev.toLocaleString()+'<\/td><td style="text-align:right;font-family:var(--mono);color:var(--success)">\u20ac'+potRev.toLocaleString()+'<\/td><td style="text-align:right;font-family:var(--mono);color:'+(delta>0?'var(--success)':'var(--text3)')+'">+'+(delta>0?'\u20ac'+delta.toLocaleString():'\u20ac0')+'<\/td><\/tr>'});var totalCur=0,totalPot=0;d.filter(function(k){return(k.vol||k.imp)>50}).forEach(function(k){var vol=k.vol||k.imp||0;var curCtr=k.pos>0?ctrByPos(k.pos):0;totalCur+=Math.round(vol*curCtr*cr*tk);totalPot+=Math.round(vol*ctrByPos(2)*cr*tk)});el.innerHTML='<div style="display:flex;gap:16px;margin-bottom:12px"><div style="padding:12px 16px;border:1px solid var(--border);border-radius:var(--radius);flex:1;text-align:center"><div style="font-family:var(--mono);font-size:1.5rem;font-weight:600">\u20ac'+totalCur.toLocaleString()+'<\/div><div style="font-size:.7rem;color:var(--text3)">Revenue actual/mes<\/div><\/div><div style="padding:12px 16px;border:1px solid var(--border);border-radius:var(--radius);flex:1;text-align:center"><div style="font-family:var(--mono);font-size:1.5rem;font-weight:600;color:var(--success)">\u20ac'+totalPot.toLocaleString()+'<\/div><div style="font-size:.7rem;color:var(--text3)">Revenue potencial Top 3<\/div><\/div><div style="padding:12px 16px;border:1px solid var(--border);border-radius:var(--radius);flex:1;text-align:center"><div style="font-family:var(--mono);font-size:1.5rem;font-weight:600;color:var(--success)">+\u20ac'+(totalPot-totalCur).toLocaleString()+'<\/div><div style="font-size:.7rem;color:var(--text3)">\u0394 Oportunidad/mes<\/div><\/div><\/div><p style="font-size:.75rem;color:var(--text3);margin-bottom:8px">Vol \u00d7 CTR<sub>pos<\/sub> \u00d7 1,85% conv \u00d7 \u20ac224,85 ticket medio<\/p><table class="kw-table"><thead><tr><th>Keyword<\/th><th style="text-align:right">Vol.<\/th><th style="text-align:right">Pos.<\/th><th style="text-align:right">\u20ac/mes actual<\/th><th style="text-align:right">\u20ac/mes Top 3<\/th><th style="text-align:right">\u0394 Oportunidad<\/th><\/tr><\/thead><tbody>'+rows.join('')+'<\/tbody><\/table>'}

function updClusters(){var el=document.getElementById('clusterContent');if(!el)return;var clMap={};KW.forEach(function(k){var cl=k.cluster||'Otros';if(!clMap[cl])clMap[cl]={kws:[],clicks:0,imp:0,gaps:0};clMap[cl].kws.push(k);clMap[cl].clicks+=k.clicks;clMap[cl].imp+=k.imp;if(k.source&&k.source.indexOf('gap:')===0)clMap[cl].gaps++});var clArr=Object.keys(clMap).sort(function(a,b){return clMap[b].imp-clMap[a].imp});document.getElementById('clusterCount').textContent=clArr.length+' clusters';var h='<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:12px">';clArr.forEach(function(name){var cl=clMap[name];var avgPos=cl.kws.filter(function(k){return k.pos>0}).length?+(cl.kws.filter(function(k){return k.pos>0}).reduce(function(s,k){return s+k.pos},0)/cl.kws.filter(function(k){return k.pos>0}).length).toFixed(1):0;var gscCount=cl.kws.filter(function(k){return!k.source}).length;var top5=cl.kws.slice().sort(function(a,b){return b.imp-a.imp}).slice(0,5);var phases={};cl.kws.forEach(function(k){phases[k.phase]=(phases[k.phase]||0)+1});var phaseStr=Object.keys(phases).sort(function(a,b){return phases[b]-phases[a]}).slice(0,3).map(function(p){return p+': '+phases[p]}).join(', ');h+='<div style="border:1px solid var(--border);border-radius:var(--radius);padding:12px;background:var(--surface)"><div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:6px"><div style="font-weight:600;font-size:.82rem">'+name+'<\/div><div style="font-family:var(--mono);font-size:.75rem;color:var(--text3)">'+cl.kws.length+' kws<\/div><\/div><div style="display:flex;gap:12px;margin-bottom:8px;font-size:.75rem;color:var(--text3)"><span>'+cl.imp.toLocaleString()+' imp<\/span><span>'+cl.clicks+' clicks<\/span><span>pos. '+avgPos+'<\/span>'+(cl.gaps?'<span style="color:var(--warn)">'+cl.gaps+' gaps<\/span>':'')+'<\/div><div style="font-size:.7rem;color:var(--text4);margin-bottom:6px">'+phaseStr+'<\/div><table class="kw-table"><thead><tr><th>Keyword<\/th><th style="text-align:right">Imp<\/th><th style="text-align:right">Pos<\/th><th style="text-align:right">Clicks<\/th><\/tr><\/thead><tbody>';top5.forEach(function(k){var src=k.source?'<span style="font-size:.6rem;color:var(--warn);margin-left:4px">[gap]<\/span>':'';h+='<tr><td>'+k.kw+src+'<\/td><td style="text-align:right;font-family:var(--mono)">'+k.imp.toLocaleString()+'<\/td><td style="text-align:right;font-family:var(--mono);color:'+(k.pos>0&&k.pos<=10?'var(--success)':k.pos>10?'var(--warn)':'var(--text3)')+'">'+(k.pos>0?k.pos.toFixed(1):'\u2014')+'<\/td><td style="text-align:right;font-family:var(--mono)">'+k.clicks+'<\/td><\/tr>'});h+='<\/tbody><\/table><\/div>'});h+='<\/div>';el.innerHTML=h}
var editArchFilter='all',editPhaseFilter='all';
function kwScore(k){var vol=k.vol||k.imp||0;var pos=k.pos||0;var kd=k.kd||0;var intent=k.intent||'';var volScore=vol>0?Math.min(100,Math.round(Math.log10(vol)*33-30)):0;var posScore=pos===0?60:pos<=3?20:pos<=5?40:pos<=10?60:pos<=20?100:pos<=50?50:30;var kdScore=kd===0?80:kd<20?100:kd<40?70:kd<60?40:20;var intMap={Transaccional:100,Comercial:80,Informacional:50,Navegacional:30,transactional:100,commercial:80,informational:50,navigational:30};var intScore=intMap[intent]||60;return Math.round(volScore*0.4+posScore*0.3+kdScore*0.2+intScore*0.1)}
...
document.addEventListener('DOMContentLoaded',function(){buildFilters();buildEditFilters();initCharts();updateAll();document.getElementById('kwSearch').addEventListener('input',function(e){sT=e.target.value;updateAll()});document.querySelector('#kwTable thead').addEventListener('click',function(e){var th=e.target.closest('th');if(!th)return;var c=th.dataset.col;if(sC===c)sD*=-1;else{sC=c;sD=['kw','intent','phase'].indexOf(c)!==-1?1:-1}updateAll()})});
<\/script></body></html>`;
}
